#!/bin/bash
# One-shot SMF AIGC Studio pane install for Hermes Desktop.
# Plugin id stays smf-aigc-studio-pane. It embeds local studio-web, not the pack builder.
#
...
REPO="${REPO:-https://github.com/smfworks/smf-aigc-studio-pane.git}"
NAME=smf-aigc-studio-pane
...
echo "Then: Ctrl+K → Open AIGC Studio pane."
echo "Local studio-web is http://127.0.0.1:5174/ (./scripts/dev-studio.sh all). The pane iframes it only when that port answers. There is no Live tab and no hosted studio URL."
